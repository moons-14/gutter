...
const progressDigest = await readFile(
...
test('canonical runtime policy is reused by migration and restore bootstrap', () => {
  ...
  assert.match(workerReadiness, /version='0014_qualified_progress_digest'/);
  assert.match(progressDigest, /public\.digest\(/);
  assert.match(progressDigest, /set search_path = pg_catalog, public/);
  assert.match(
    progressDigest,
...
test('worker policy excludes auth, ACL, and user-state DML while preserving queue/catalog access', () => {
  assert.doesNotMatch(
    policy,
    /grant [^;]+ on[^;]*(?:"user"|library_access_grants|user_target_state|user_progress|gutter_acl_audit)[^;]* to gutter_worker/,
  );
  assert.match(
    policy,
