...
const policy = await readFile(new URL('../packages/db/drizzle/0011_runtime_acl_bootstrap.sql', import.meta.url), 'utf8');
...
test('canonical runtime policy is reused by migration and restore bootstrap', () => {
  ...
  assert.match(bootstrap, /0011_runtime_acl_bootstrap\.sql/);
  assert.match(migrate, /0011_runtime_acl_bootstrap\.sql/);
});
