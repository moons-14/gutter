...
function validate(value, rule, path = '$', root = schema) {
  ...
  if (rule.type === 'object' && rule.properties) {
    ...
    for (const [key, child] of Object.entries(rule.properties))
      if (Object.hasOwn(value, key)) validate(value[key], child, `${path}.${key}`, root);
  }
  if (rule.type === 'array' && rule.items)
    for (const [i, child] of value.entries()) validate(child, rule.items, `${path}[${i}]`, root);
}
...
if (report.status === 'unavailable') {
  ...
  for (const key of [
    ...
    assert.ok(Object.hasOwn(report, key), `unavailable missing ${key}`);
...
assert.equal(
  new Set(['first', 'noChange', 'changed'].map((name) => report.worker.runs[name].pgBossJobId))
    .size,
  3,
);
...
if (!unavailableMode)
  assert.equal(
    report.baselineComparison.baselineSha256,
