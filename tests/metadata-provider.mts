...
test('metadata provider rejects incompatible capabilities and bounds every response body', async () => {
  let restore = mockFetch(
    (url) => new Response(JSON.stringify(url.pathname === '/v1/health' ? {} : { version: 999 })),
  );
  await assert.rejects(
    lookupSidecar('http://metadata-sidecar', request, {
      token: 'token',
      timeoutMs: 1000,
      payloadBytes: 1024,
    }),
...
test('metadata provider rejects malformed candidates and an already-aborted request never fetches', async () => {
  assert.equal(
    validLookupResponse({ version: 1, candidates: [{ ...candidate, values: [] }] }),
    false,
  );
  assert.equal(
    validLookupResponse({
      version: 1,
      candidates: [{ ...candidate, provenance: JSON.parse('{"__proto__":"unsafe"}') }],
    }),
    false,
  );
...
test('metadata provider rejects invalid or extra request fields before fetch', async () => {
  ...
  await assert.rejects(
    lookupSidecar(
      'http://metadata-sidecar',
      { ...request, canonicalIdentity: 'A'.repeat(64) },
      { token: 'token', timeoutMs: 1000, payloadBytes: 1024 },
    ),
  ...
  await assert.rejects(
    lookupSidecar(
      'http://metadata-sidecar',
      { ...request, searchTerms: ['../../private'] },
      { token: 'token', timeoutMs: 1000, payloadBytes: 1024 },
    ),
    /invalid_lookup_request/,
  );
  await assert.rejects(
    lookupSidecar('http://metadata-sidecar', { ...request, extra: true } as typeof request, {
      token: 'token',
      timeoutMs: 1000,
      payloadBytes: 1024,
    }),
...
test('sidecar token paths stay inside the canonical secrets directory', async () => {
  ...
  await assert.rejects(
    sidecarToken(join(secrets, '..', 'outside-token'), secrets),
    /invalid GUTTER_METADATA_SIDECARS_JSON/,
  );
  await assert.rejects(
    sidecarToken(join(prefixedSecrets, 'token'), secrets),
    /invalid GUTTER_METADATA_SIDECARS_JSON/,
  );
  await assert.rejects(
    sidecarToken(join(secrets, 'escape'), secrets),
    /invalid GUTTER_METADATA_SIDECARS_JSON/,
  );
});
...
test('dispatcher retries only until abort and bounds all sidecars', async () => {
  ...
  let attempts = 0;
  await assert.rejects(
    dispatchConfiguredSidecars(
      { ...config, sidecars: [config.sidecars[0]] },
      'root',
  ...
  const merged = mergeCandidates([
    {
      ...
      providerPriority: 2,
      configOrder: 0,
      values: { title: 'later', keep: null },
      provenance: { title: 'later' },
    },
    {
      providerId: 'first',
      providerPriority: 1,
      configOrder: 1,
      values: { title: 'first', keep: 'kept' },
      provenance: { title: 'first', keep: 'first' },
    },
    {
      providerId: 'tie-break',
      providerPriority: 1,
      configOrder: 2,
      values: { title: 'ignored', keep: 'ignored' },
      provenance: {},
    },
  ]);
