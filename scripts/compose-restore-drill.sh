...
insert into gutter_public_api_tokens(id,user_id,token_hash,label,scopes) values ('00000000-0000-4000-8000-000000000027','drill-admin',decode(repeat('a',64),'hex'),'drill PAT',array['catalog:read']);
...
begin
  ...
  if not has_table_privilege('gutter_api','public.gutter_public_api_tokens','SELECT') then raise exception 'api PAT read denied'; end if;
  if not has_table_privilege('gutter_api','public.gutter_public_api_tokens','INSERT') then raise exception 'api PAT write denied'; end if;
  if not has_table_privilege('gutter_api','public.public_progress_source_items','SELECT') then raise exception 'api public progress view denied'; end if;
  if not has_table_privilege('gutter_api','public.public_reader_source_pages','SELECT') then raise exception 'api public reader view denied'; end if;
...
metrics_status=$(docker compose -p "$project_b" -f compose.yaml -f "$root/override.yaml" exec -T web sh -c 'wget -q -S -O /dev/null http://127.0.0.1:8080/api/metrics 2>&1 | awk '\''/^[[:space:]]*HTTP\/[0-9.]+[[:space:]][0-9][0-9][0-9]([[:space:]]|$)/ { if ($2 ~ /^[0-9][0-9][0-9]$/) { print $2; exit } }'\''' | tr -d '\r')
...
echo "restore drill passed: project=$project_b durable_user=1 acl_audit=1 user_state=1 public_pat=1 source_sha256=$source_sha cache_sha_before=$cache_sha cache_sha_after=$cache_sha_regenerated source=read-only validation=$validation_state/$validation_valid"
