...
`migrate` is the sole schema applier. The idempotent `0002_source_inventory` migration adds
rebuildable discovery runs, source items, and page locators; API/worker reject incompatible schemas. Before pg-boss starts, the
...
M1 has no watcher, reader, auth, mutable root API, ComicInfo support, CRC/image decoding, or external provider. Pino JSON
